...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (Stage.curStage == "hexw" && reactiveNotes.length != 0)
		{
			...
			{
				Debug.logTrace("doing light " + reactive[1] + " with time of " + reactive[2] + " space " + space);
				...
				Stage.swagBacks['' + reactive[1]].alpha = 0.8;
				activeTweens[reactive[1]].push(FlxTween.tween(Stage.swagBacks['' + reactive[1]], {alpha: 0}, space, {
		...
		if (generatedMusic && currentSection != null)
		{
				...
				&& Math.floor(currentSection.startTime) != lastUpdatedSectionId)
			{
				...
				if (SONG.song.toLowerCase() == "cooling")
				{
					...
					{
						var  = Stage.swagBacks['break0'];
						var o = Stage.swagBacks['break1'];
						.x = dad.x - 25;
						.y = -dad.y - 160;
						FlxTween.tween(, {alpha: 1}, 0.45);
						FlxTween.tween(o, {alpha: 0}, 0.45);
					}
					else if (Stage.curStage == "hexw")
					{
						var  = Stage.swagBacks['break1'];
						var o = Stage.swagBacks['break0'];
						if (.alpha != 0 || o.alpha != 0)
						{
							FlxTween.tween(, {alpha: 0}, 0.45);
							FlxTween.tween(o, {alpha: 0}, 0.45);
						}
					}
				}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && Conductor.songPosition >= daNote.strumTime && daNote.visible)
				{
					...
					{
						if (daNote.InLine != daNote.parent.children.length - 1)
						{
									...
									else
									{
										...
										else
										{
											if (!daNote.wasGoodHit
												...
												&& daNote.InLine != daNote.parent.children.length)
											{
												// health -= 0.05; // give a health punishment for failing a LN
												trace("hold fell over at " + daNote.InLine);
